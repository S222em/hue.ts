...
export class REST {
	...
	public async get(route: Route, data?: Record<string, any>): Promise<ApiReturnGet<any>> {
		return (await this.request('GET', route, data)) as ApiReturnGet<any>;
	}

	public async put(route: Route, data?: Record<string, any>): Promise<ApiReturnPut<any>> {
		return (await this.request('PUT', route, data)) as ApiReturnPut<any>;
	}

	public async post(route: Route, data?: Record<string, any>): Promise<ApiReturnPost<any>> {
		return (await this.request('POST', route, data)) as ApiReturnPost<any>;
	}

	public async delete(route: Route, data?: Record<string, any>): Promise<ApiReturnDelete<any>> {
		return (await this.request('DELETE', route, data)) as ApiReturnDelete<any>;
	}

	public async request(method: 'GET' | 'PUT' | 'POST' | 'DELETE', route: Route, data?: Record<string, any>) {
