...
export function createLightPutPayload(options: LightEditOptions): RESTPayload {
	return {
		...
		timed_effects: options.timedEffects
			...
			: undefined,
		dimming: {
			brightness: options.brightness ?? options.color?.z,
		},
		dimming_delta: options.brightnessDelta
			...
			: undefined,
		color_temperature: options.colorTemperature
			? {
					mirek: options.colorTemperature,
			  }
			: undefined,
		...
		color: options.color
			? {
					xy: { x: options.color.x, y: options.color.y },
			  }
			: undefined,
